...
$dbname = 'amanimed';  // your database name
...
// Fetch all unique emails from the exercise_data table
$query = "SELECT DISTINCT email FROM exercise_data";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])) {
    ...
    // Update the record in the exercise_data table
    $update_query = "UPDATE exercise_data SET exercise_type = ?, duration = ?, date = ?, status = ? WHERE id = ?";
    $stmt = $conn->prepare($update_query);
    $stmt->bind_param("ssssi", $exercise_type, $duration, $date, $status, $id);
...
if (isset($_GET['email'])) {
    ...
    $query = "SELECT * FROM exercise_data WHERE email = ?";
...
<head>
    ...
    <title>Exercise Data - View & Update</title>
...
<body class="bg-gray-100 p-6">
    ...
    <!-- Table displaying exercise data for selected email -->
    <?php if (isset($_GET['email']) && !empty($data)) { ?>
        <h2 class="text-2xl font-semibold text-gray-800 my-6">Exercise Data for <?php echo htmlspecialchars($selected_email); ?></h2>

        <table class="min-w-full bg-white shadow-md rounded-lg">
            <thead class="bg-gray-200">
                <tr>
                    <th class="p-4">ID</th>
                    <th class="p-4">Exercise Type</th>
                    <th class="p-4">Duration (minutes)</th>
                ...
                <?php foreach ($data as $row) { ?>
                    <tr>
                        <form method="POST" action="">
                            ...
                            <td class="p-4">
                                <input type="text" name="exercise_type" value="<?php echo $row['exercise_type']; ?>" class="p-2 border border-gray-300 rounded" required>
                            </td>
                            <td class="p-4">
                                <input type="number" name="duration" value="<?php echo $row['duration']; ?>" class="p-2 border border-gray-300 rounded" required>
                            </td>
                            <td class="p-4">
    ...
    <?php } elseif (isset($_GET['email']) && empty($data)) { ?>
        <p class="text-red-500">No exercise data available for this email.</p>
    <?php } ?>
